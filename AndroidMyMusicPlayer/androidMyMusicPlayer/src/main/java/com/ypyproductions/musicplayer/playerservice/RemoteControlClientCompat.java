...
public class RemoteControlClientCompat {
	...
	public RemoteControlClientCompat(PendingIntent pendingIntent, Looper looper) {
		...
		try {
			mActualRemoteControlClient = sRemoteControlClientClass.getConstructor(PendingIntent.class, Looper.class).newInstance(pendingIntent, looper);
		}
		catch (Exception e) {
	...
	public class MetadataEditorCompat {
		 ...
		 * 
		 * @param key
		 *            The identifier of a the metadata field to set. Valid
		 *            values are
		 ...
		 *            .
		 * @param value
		 *            The text for the given key, or {@code null} to signify
		 ...
		 * 
		 * @param key
		 *            the identifier of the bitmap to set. The only valid value
	 ...
	 *            metadata that was previously applied to the
