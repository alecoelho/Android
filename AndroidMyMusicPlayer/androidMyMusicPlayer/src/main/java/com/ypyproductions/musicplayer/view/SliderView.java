package com.ypyproductions.musicplayer.view;
...
public class SliderView extends CustomView {
	...
	private void init(){
		...
		mPaintIndicator.setColor(backgroundColor);
		mPaintIndicator.setAntiAlias(true);
		
		mPaintNormal = new Paint();
		mPaintNormal.setColor(Color.parseColor("#B0B0B0"));
		mPaintNormal.setStrokeWidth(ViewUtils.dpToPx(2, getResources()));
		
		mPaintActive = new Paint();
		mPaintActive.setColor(backgroundColor);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (value == min) {
			...
			mTempCanvas.drawCircle(ViewHelper.getX(ball) + ball.getWidth() / 2, ViewHelper.getY(ball) + ball.getHeight() / 2, ball.getWidth() / 2, mTransparentPaint);
		...
		if (press && !showNumberIndicator) {
			canvas.drawCircle(ViewHelper.getX(ball) + ball.getWidth() / 2, getHeight() / 2, getHeight() / 3, mPaintIndicator);
		}
		invalidate();
	...
	class Indicator extends RelativeLayout {
		...
		protected void onDraw(Canvas canvas) {
			...
			canvas.drawCircle(ViewHelper.getX(ball) + ViewUtils.getRelativeLeft((View) ball.getParent()) + ball.getWidth() / 2, y, size, paint);
			if (animate && size >= finalSize)
				animate = false;
			if (animate == false) {
				ViewHelper.setX(numberIndicator.numberIndicator, (ViewHelper.getX(ball) + ViewUtils.getRelativeLeft((View) ball.getParent()) + ball.getWidth() / 2) - size);
